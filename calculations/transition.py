...
def transition(
    ...
    # Если сужение
    if calcversion == "22":
        print("Расчет скорости по версии 22, по второму сечению")
        # TODO в программе скорость считается по второму сечению (по крайней мере, для расширения)
        # а должна по первому сечению
        v_base = flow / 3600 / s2
    elif calcversion is None:
        v_base = flow / 3600 / s1
    else:
